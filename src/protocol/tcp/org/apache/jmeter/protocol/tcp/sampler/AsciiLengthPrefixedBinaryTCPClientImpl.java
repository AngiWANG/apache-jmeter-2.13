...
/**
 * Implements binary ascii-length-prefixed binary data.
 * This is used in ISO8583 for example.
 */
public class AsciiLengthPrefixedBinaryTCPClientImpl extends TCPClientDecorator {
    private static final Logger log = LoggingManager.getLoggerForClass();

    private final int lengthPrefixLen = JMeterUtils.getPropDefault("tcp.binarylength.prefix.length", 2); // $NON-NLS-1$

    public AsciiLengthPrefixedBinaryTCPClientImpl() {
...
//      os.write(intToByteArray(s.length()/2,lengthPrefixLen));
    	// 前面追加长度值（ascii）
    	os.write(stuffString(s.length()/2+"",lengthPrefixLen,true,'0').getBytes());
...
//              msgLen = byteArrayToInt(lengthBuffer);
            	// 解析长度值（ascii）
            	msgLen = Integer.parseInt(new String(lengthBuffer));
                msg = new byte[msgLen];
